...
public abstract class AbstractDealDamageHandler extends AbstractMaplePacketHandler {
    ...
    protected AttackInfo parseDamage(LittleEndianAccessor lea, MapleCharacter chr, boolean ranged, boolean magic) {
        ...
        if (ret.skill == FPWizard.POISON_BREATH) {
            ret.numAttacked = 4;
            int oid = lea.readInt();
            lea.skip(14);
            MapleMonster mainMonster = chr.getMap().getMonsterByOid(oid);
            Point p = mainMonster.getPosition();
            MapleMonster[] nearbyMonsters = chr.getMap().getMonstersByPoint(p, ret.numAttacked - 1);
            MapleMonster[] finalNearbyMonsters = new MapleMonster[4];
            finalNearbyMonsters[0] = mainMonster;
            for (int i = 0; i < ret.numAttacked - 1; i++) {
                finalNearbyMonsters[i + 1] = nearbyMonsters[i];
            }
            int damage = lea.readInt();

            for (int i = 0; i < ret.numAttacked; i++) {
                List<Integer> allDamageNumbers = new ArrayList<>();
                MapleMonster monster = finalNearbyMonsters[i];
                ...
                lea.skip(4);
                int monsteroid = monster.getObjectId();
                System.out.println("Num AllDamageNumbers : " + allDamageNumbers);
                ret.allDamage.put(Integer.valueOf(monsteroid), allDamageNumbers);
            }
        } else {
            for (int i = 0; i < ret.numAttacked; i++) {
                int oid = lea.readInt();
